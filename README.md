...
.\OnelapSyncStrava.exe upload-fit downloads\raw.fit
...
如果只想验证 Strava 网页上传链路，不想登录顽鹿或改动 `state.json`，可以直接上传一个本地 FIT 活动文件：

```bash
./OnelapSyncStrava upload-fit downloads/raw.fit
```

也可以传目录，程序会上传该目录当前层级下的 `.fit` 文件：

```bash
./OnelapSyncStrava upload-fit downloads
```

该命令会使用当前 `strava.upload_method`。默认 Web 模式下，它会先访问 `https://www.strava.com/upload/select` 获取 CSRF token，再把活动文件作为 `files[]` 上传到 Strava 网页上传接口。默认每批最多 15 个文件，批间等待 2 分钟，这是按连续上传约 30 个文件触发限速的反馈做的保守估计；可以用 `-upload-batch` 和 `-upload-delay` 覆盖。
...
历史补传如果一次超过 15 条，默认会分批上传并等待，避免 Strava 网页上传触发 `429 Too Many Requests`：

```bash
./OnelapSyncStrava sync --since=1y -upload-batch=15 -upload-delay=2m
```

如果你确认自己的账号可以承受更高频率，可以把等待时间调短，例如 `-upload-delay=30s`；如果仍然触发限速，就改成 `-upload-delay=5m` 再跑。
